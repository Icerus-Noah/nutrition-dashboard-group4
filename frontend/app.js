...
async function loadSecurityStatus() {
   try {
       ...
       if (!res.ok) throw new Error(`Failed to fetch security status: ${res.status}`);
       const data = await res.json();
...
async function loadUserInfo() {
   ...
   if (currentUser) {
       userInfo.innerHTML = `
           Signed in as <strong>${currentUser.userDetails || currentUser.identityProvider}</strong>
<br>
           Provider: ${currentUser.identityProvider || "Unknown"}
       `;
   } else {
